...
.exphint{margin-left:auto;font-size:12px}
 ...
 .mytimelinefilter{appearance:none;background:transparent;border:0;border-bottom:2px solid transparent;border-radius:0;color:var(--mut);padding:4px 1px 5px;font:inherit;font-size:11px;font-weight:650;cursor:pointer;transition:color .12s,border-color .12s}
 ...
 .mytimelinefilterpanel .timelinefilterreset{width:100%;margin-top:8px;background:transparent;border:1px solid var(--line2);color:var(--mut);border-radius:6px;padding:6px;cursor:pointer}
...
let _myTimelineFilter=(localStorage.getItem('tvmateTimelineFilter')||'all');
let _myTimelineSettings=(function(){try{return Object.assign({recent:true,live:true,upcoming:true,maxPerCategory:0},JSON.parse(localStorage.getItem('tvmateTimelineSettings')||'{}'));}catch(e){return {recent:true,live:true,upcoming:true,maxPerCategory:0};}})();
...
function timelineControlsHtml(){
  const kinds=[['all','All'],['show','Shows'],['movie','Movies'],['game','Games'],['sport','Sports'],['f1','Racing']];
  let h='<div class="mytimelinecontrols">';
  for(const k of kinds)h+='<button class="mytimelinefilter '+k[0]+(_myTimelineFilter===k[0]?' on':'')+'" onclick="setTimelineFilter(\''+k[0]+'\')">'+esc(tr(k[1]))+'</button>';
  ...
  h+='<div class="mytimelinefilterpanel hide"><h4>'+esc(tr('Timeline settings'))+'</h4><div class="timelinechecks">'
    ...
    +'<button class="timelinefilterreset" onclick="resetTimelineSettings()">'+esc(tr('Reset to default'))+'</button></div></div>';
  return h;
}
function setTimelineFilter(kind){_myTimelineFilter=['all','show','movie','game','sport','f1'].includes(kind)?kind:'all';localStorage.setItem('tvmateTimelineFilter',_myTimelineFilter);renderMyListTimeline();}
function toggleTimelineSettings(btn){const panel=btn.closest('.mytimelinecontrols').querySelector('.mytimelinefilterpanel');panel.classList.toggle('hide');}
function setTimelineSetting(input){const key=input.dataset.setting;if(key==='maxPerCategory')_myTimelineSettings[key]=Math.max(0,Number(input.value||0));else _myTimelineSettings[key]=!!input.checked;localStorage.setItem('tvmateTimelineSettings',JSON.stringify(_myTimelineSettings));renderMyListTimeline();}
function resetTimelineSettings(){_myTimelineSettings={recent:true,live:true,upcoming:true,maxPerCategory:0};localStorage.setItem('tvmateTimelineSettings',JSON.stringify(_myTimelineSettings));renderMyListTimeline();}

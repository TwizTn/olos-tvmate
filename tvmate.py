...
async function play(id){try{const u=await api('/api/hls?id='+encodeURIComponent(id));if(u.hls)location.href=u.hls}catch(e){alert('Kan ikke åpne strømmen.')}}async function vlc(id){try{await api('/api/play',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({stream_id:id})})}catch(e){alert('Kunne ikke starte VLC på PC-en.')}}
...
async function loadSport(){try{const r=await api('/api/my_teams');$('sportRows').innerHTML=(r.fixtures||[]).filter(x=>!x.is_finished).map(f=>fixtureCard(f)).join('')||empty('Ingen kommende kamper.')}catch(e){$('sportRows').innerHTML=empty('Kunne ikke laste kamper.')}}
...
function mediaTile(x){return '<div class="tile">'+(x.cover?'<img src="'+esc(x.cover)+'" loading="lazy">':'<div style="aspect-ratio:2/3"></div>')+'<div class="title">'+esc(x.name||x.show_name||'')+'</div><div class="meta">'+esc(x.year||x.episode_name||'')+'</div></div>'}

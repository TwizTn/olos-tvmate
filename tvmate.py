...
def fetch_vg_channel_schedule(slug, date_key):
    ...
    for listing in schedule.get("listings") or []:
        ...
        title = " · ".join(value for index, value in enumerate(names)
...
<main>
  ...
  <section id="teamsView" class="hide">
    ...
    <div id="sportsHubPage" class="teamswrap">
      ...
      <div class="teamsmain">
        <div class="sportsactions"><h2 class="colh" data-i18n="Sports">Sports</h2><button id="leagueEditBtn" type="button" class="ghost" aria-expanded="false" onclick="toggleLeaguePicker(this)">&#9998; <span data-i18n="Edit leagues & cups">Edit leagues &amp; cups</span></button><div id="leaguePicker" class="leaguepicker hide"></div></div>
      ...
      <div id="settingsProfile" class="settingspanel">
       <div class="settingsgroup" data-settings-panel="profile">
        ...
        <div class="grid2">
          ...
          <div><label data-i18n="Sports layout">Sports layout</label>
            <select id="s_sportslayout"><option value="current" data-i18n="Current Sports">Current</option><option value="broadcast" data-i18n="Broadcast Deck">Broadcast Deck</option><option value="agenda" data-i18n="TV Agenda">TV Agenda</option><option value="hub" data-i18n="Sports Hub">Sports Hub</option><option value="timeline" data-i18n="Timeline Rail">Timeline Rail</option></select>
            <div class="muted" style="margin-top:6px" data-i18n="Changes the arrangement of your Sports page only.">Changes the arrangement of your Sports page only.</div></div>
          <div><label data-i18n="Match page layout">Match page layout</label>
            <select id="s_matchlayout"><option value="balanced" data-i18n="Balanced">Balanced</option><option value="channel-first" data-i18n="Channel first">Channel first</option><option value="live-centre" data-i18n="Live centre">Live centre</option></select>
            <div class="muted" style="margin-top:6px" data-i18n="Changes the dedicated page opened when you select a fixture.">Changes the dedicated page opened when you select a fixture.</div></div>
...
let _lang='en';
const _I18N={
  "Sports layout":"Sportoppsett","Current Sports":"N\u00e5v\u00e6rende","Broadcast Deck":"TV-studio","TV Agenda":"TV-program","Sports Hub":"Sportsoversikt","Timeline Rail":"Tidslinje","Changes the arrangement of your Sports page only.":"Endrer bare oppsettet p\u00e5 sportsiden din.",
  "Edit layout":"Rediger oppsett","Choose a layout. Your selection is saved immediately.":"Velg et oppsett. Valget lagres med en gang.","Layout saved.":"Oppsett lagret.","Could not save layout.":"Kunne ikke lagre oppsettet.","Time-ordered overview":"Tidsordnet oversikt","Even content sections":"Jevne innholdsseksjoner","Featured content first":"Fremhevet innhold først","Sidebar dashboard":"Sidepaneloversikt","Familiar two-column view":"Velkjent visning med to kolonner","TV-focused fixture cards":"TV-fokuserte kampkort","Compact schedule rows":"Kompakte programrader","Dashboard columns":"Oversiktskolonner","Wide chronological list":"Bred kronologisk liste","Teams, channels and details":"Lag, kanaler og detaljer","Put playable channels first":"Vis spillbare kanaler først","Live-status dashboard":"Oversikt med livestatus",
  ...
  "Match page layout":"Kampsideoppsett","Channel first":"Kanaler først","Live centre":"Livesenter","Changes the dedicated page opened when you select a fixture.":"Endrer den egne siden som åpnes når du velger en kamp.","Channel matches":"Kanaltreff","Watch this match":"Se denne kampen","Official broadcasters":"Offisielle TV-kanaler","Channel matching":"Kanalsøk","Updates quietly every 15 minutes":"Oppdateres stille hvert 15. minutt",
...
const _LAYOUT_EDITORS={
  profile:{key:'mylist_layout',title:'Profile layout',choices:[['timeline','Now Timeline','Time-ordered overview'],['balanced','Balanced','Even content sections'],['spotlight','Spotlight','Featured content first'],['hub','Profile Hub','Sidebar dashboard']]},
...
async function choosePageLayout(value,btn){
  ...
  try{const r=await api('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!r.ok)throw new Error('save failed');applyProfileConfig(payload);if(spec.key==='mylist_layout'){s_mylistlayout.value=value;ep_layout.value=value;}else if(spec.key==='sports_layout'){s_sportslayout.value=value;ep_sportslayout.value=value;}else{s_matchlayout.value=value;ep_matchlayout.value=value;}closeLayoutEditor();toast(tr('Layout saved.'));}catch(e){layoutPickerChoices.querySelectorAll('button').forEach(button=>button.disabled=false);toast(tr('Could not save layout.'));}
}
...
function applySportsLayout(){
  const view=document.getElementById('teamsView');if(!view)return;
  const allowed=['current','broadcast','agenda','hub','timeline'];
  const layout=allowed.includes(_profileConfig.sports_layout)?_profileConfig.sports_layout:'current';
  allowed.forEach(name=>view.classList.remove('sports-layout-'+name));
  view.classList.add('sports-layout-'+layout);
}
function applyProfileConfig(c){
  ...
  applyMyListLayout();
  applySportsLayout();
  if(!matchView.classList.contains('hide')&&_matchFixture)renderMatchPage();
}

let _favTeamSet=new Set(),_favTeamRows=[],_myTeamFixtures=[],_sportsTopFixtures=[],_sportsVisibleFixtures=[],_sportsAvailabilityTimer=null,_sportsAvailabilityRequest=0,_myTeamsBackgroundRefresh=false,_myTeamsLastRefresh=0,_selectedTeamName='',_selectedTeamRow=null,_selectedTeamProfile=null,_teamProfileReq=0,_teamDeepLink=null,_fixtureSearchTeamId='';
...
function teamFixtureCard(f,live,deepLink){
  ...
  const favorite=(f.favorite_teams||[]).length||_favTeamRows.some(t=>fixtureBelongsToTeam(f,t.name));
  return '<div class="teamfixture hastv'+(deepLink?' teamfixturerow':'')+(live?' livefixture':'')+(favorite?' favoritefixture':'')+(hasChannels?' haschannels':'')+'"'+fixtureAttrs+'><div class="teamfixtureteams"><span class="teamfixtureside">'+homeLogo+esc(f.home)+'</span><span class="teamfixturevs">v</span><span class="teamfixtureside">'+awayLogo+esc(f.away)+'</span>'
...
function openMyTeamsFixture(target){
  ...
  if(selected)openMatchFromCard(selected);
}
...
async function refreshMatchChannels(btn,force){
  ...
  try{const result=await api('/api/sports_event_channels',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fixture:_matchFixture,force:!!force})});if(!result.error){Object.assign(_matchFixture,result);applySportsAvailability({[sportsFixtureKey(_matchFixture)]:result});renderMatchPage();const updated=document.getElementById('matchUpdated');if(updated)updated.textContent=tr('Channel matches refreshed.');}}catch(e){}finally{if(btn){btn.disabled=false;btn.innerHTML=old;applyLang();}if(_matchRefreshTimer)clearTimeout(_matchRefreshTimer);if(!matchView.classList.contains('hide'))_matchRefreshTimer=setTimeout(()=>refreshMatchChannels(null,true),15*60*1000);}
}
...
async function saveSettings(){
  const body={xtream_host:s_host.value,xtream_user:s_user.value,
    ...
    profile_emblem:_selectedEmblem,mylist_layout:s_mylistlayout.value,sports_layout:s_sportslayout.value,match_layout:s_matchlayout.value,football_enabled:s_football.checked,
...
function racingChannelSections(channels){
  ...
  let h='';
  if(definite.length)h+='<div class="muted">'+esc(tr('Confirmed racing channels'))+'</div>'+definite.map(racingChannelLine).join('');
...
document.addEventListener('click',function(e){
  ...
  if(teamFixture&&!e.target.closest('.btnplay,.btnvlc,.bchead')){openMatchFromCard(teamFixture);return;}
...
# Independent phone client. It deliberately shares only the HTTP API with the
# desktop UI: no desktop markup, grids, sidebars, renderers, or CSS are reused.
MOBILE_PAGE = r'''<!doctype html>
<html lang="no"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">
<meta name="theme-color" content="#090d12"><title>Olo Mobile</title>
<style>
:root{color-scheme:dark;--bg:#090d12;--panel:#121821;--panel2:#192230;--line:#273245;--text:#f4f7fb;--muted:#929eb1;--blue:#4e8df7;--green:#45d483;--red:#f06e74;--orange:#ef8c35;--safe:env(safe-area-inset-bottom,0px)}
*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:var(--bg);color:var(--text);font-family:system-ui,-apple-system,Segoe UI,sans-serif}button,input,select{font:inherit;color:inherit}button{cursor:pointer}.app{min-height:100dvh;padding-bottom:calc(84px + var(--safe))}.top{position:sticky;top:0;z-index:20;display:flex;align-items:center;gap:12px;padding:14px 16px;background:rgba(9,13,18,.94);backdrop-filter:blur(16px);border-bottom:1px solid var(--line)}.brand{font-size:20px;font-weight:850;letter-spacing:-.4px}.brand i{font-style:normal;color:var(--blue)}.grow{flex:1}.iconbtn,.pill,.action{border:1px solid var(--line);background:var(--panel2);border-radius:14px;padding:10px 13px}.iconbtn{width:44px;height:44px;padding:0;font-size:21px}.screen{display:none;padding:18px 15px 30px;max-width:680px;margin:auto}.screen.on{display:block}.hero{padding:8px 2px 18px}.eyebrow{color:var(--blue);font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:1.2px}.hero h1{font-size:30px;line-height:1.06;margin:7px 0 8px;letter-spacing:-1px}.muted{color:var(--muted)}.section{margin:17px 0 26px}.sectionhead{display:flex;align-items:center;gap:10px;margin:0 2px 11px}.sectionhead h2{font-size:17px;margin:0}.card{background:linear-gradient(145deg,var(--panel),#10161e);border:1px solid var(--line);border-radius:19px;overflow:hidden;margin-bottom:11px}.row{display:flex;align-items:center;gap:12px;padding:13px}.row+.row{border-top:1px solid var(--line)}.art{width:54px;height:54px;flex:0 0 54px;border-radius:14px;background:#202b3a;object-fit:cover}.art.poster{height:78px;border-radius:11px}.title{font-weight:760;line-height:1.25}.meta{color:var(--muted);font-size:13px;margin-top:4px;line-height:1.35}.tag{font-size:11px;font-weight:800;color:var(--green);border:1px solid #286441;background:#10251a;border-radius:8px;padding:4px 7px}.live{color:#fff;background:#9b2631;border-color:#dc4d59}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:9px}.action{padding:8px 11px;border-radius:11px;font-size:13px}.action.primary{background:#1552b7;border-color:#286edb}.action.vlc{background:#a94d0c;border-color:#d76718}.search{display:flex;gap:9px;margin-bottom:14px}.search input,.field input,.field select{min-width:0;width:100%;border:1px solid var(--line);background:#0d1219;border-radius:15px;padding:14px 15px;outline:none}.search input:focus,.field input:focus{border-color:var(--blue)}.search button{border:0;background:var(--blue);border-radius:15px;padding:0 18px;font-weight:800}.chips{display:flex;gap:8px;overflow:auto;padding:2px 1px 10px;scrollbar-width:none}.chip{white-space:nowrap;border:1px solid var(--line);background:var(--panel);border-radius:999px;padding:9px 13px}.chip.on{background:#173259;border-color:#3478df}.empty{padding:30px 18px;text-align:center;color:var(--muted);border:1px dashed var(--line);border-radius:18px}.fixture{padding:15px}.teams{display:flex;align-items:center;gap:8px;font-weight:820;font-size:17px}.versus{color:var(--muted);font-weight:500}.fixture .meta{margin-top:7px}.channelgroup{border-top:1px solid var(--line);padding:12px 14px}.channelgroup summary{font-weight:750;cursor:pointer}.channel{display:flex;align-items:center;gap:10px;padding:10px 0}.channel+.channel{border-top:1px solid #202a39}.channel .art{width:38px;height:38px;flex-basis:38px;border-radius:9px}.quality{font-size:10px;color:#b8c3d3}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:11px}.tile{min-width:0;background:var(--panel);border:1px solid var(--line);border-radius:18px;padding:12px}.tile img{width:100%;aspect-ratio:2/3;object-fit:cover;border-radius:12px;background:#202b3a}.tile .title{margin-top:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.categories{display:grid;gap:8px}.category{display:flex;align-items:center;gap:10px;width:100%;text-align:left;border:1px solid var(--line);background:var(--panel);border-radius:15px;padding:13px}.category span:last-child{margin-left:auto;color:var(--muted)}.field{margin:13px 0}.field label{display:block;color:var(--muted);font-size:13px;margin:0 0 7px 3px}.save{width:100%;border:0;background:var(--blue);border-radius:15px;padding:14px;font-weight:850}.notice{padding:12px 14px;border-radius:14px;background:#132035;color:#c9d8f0;margin-bottom:12px}.bottom{position:fixed;z-index:30;left:0;right:0;bottom:0;height:calc(72px + var(--safe));padding:7px 8px var(--safe);display:grid;grid-template-columns:repeat(5,1fr);background:rgba(8,12,17,.96);backdrop-filter:blur(18px);border-top:1px solid var(--line)}.nav{border:0;background:transparent;color:var(--muted);border-radius:13px;font-size:11px;font-weight:700;padding:5px 2px}.nav b{display:block;font-size:21px;line-height:25px}.nav.on{color:#fff;background:#162944}.sheetback{display:none;position:fixed;z-index:40;inset:0;background:#0009}.sheetback.on{display:block}.sheet{position:absolute;left:0;right:0;bottom:0;background:#121923;border-radius:24px 24px 0 0;padding:10px 15px calc(22px + var(--safe));max-height:78dvh;overflow:auto}.handle{width:42px;height:5px;border-radius:5px;background:#405069;margin:2px auto 17px}.sheetitem{width:100%;display:flex;gap:13px;align-items:center;border:0;border-bottom:1px solid var(--line);background:transparent;padding:15px 8px;text-align:left;font-weight:750}.sheetitem b{font-size:22px;width:30px}.loader{width:24px;height:24px;border:3px solid #344055;border-top-color:var(--blue);border-radius:50%;animation:spin .8s linear infinite;margin:30px auto}@keyframes spin{to{transform:rotate(360deg)}}
.tile.tileplay{cursor:pointer;position:relative}.tile.tileplay:active{transform:scale(.98)}.tilebadge{position:absolute;top:18px;right:18px;width:34px;height:34px;display:flex;align-items:center;justify-content:center;background:rgba(21,82,183,.92);border-radius:50%;font-size:15px;color:#fff;pointer-events:none}
.mplayer{position:fixed;inset:0;z-index:60;background:#000c;display:flex;align-items:center;justify-content:center;padding:12px}.mplayer.hide{display:none}.mplayerbox{width:100%;max-width:680px;background:#0c1017;border:1px solid var(--line);border-radius:18px;overflow:hidden}.mplayerbar{display:flex;align-items:center;gap:10px;padding:11px 14px;border-bottom:1px solid var(--line)}.mplayerbar span{flex:1;font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.mplayerclose{border:0;background:transparent;color:var(--muted);font-size:26px;line-height:1;padding:0 4px}.mplayer video{width:100%;aspect-ratio:16/9;background:#000;display:block}.mplayermsg{padding:8px 14px;color:var(--muted);font-size:13px;min-height:18px}.mplayeractions{display:flex;gap:8px;padding:0 14px 14px}.mplayeractions .action{flex:1;text-align:center}
@media(min-width:700px){.bottom{left:50%;right:auto;width:680px;transform:translateX(-50%);border:1px solid var(--line);border-bottom:0;border-radius:20px 20px 0 0}.grid{grid-template-columns:repeat(3,1fr)}}
</style></head><body><div class="app">
<header class="top"><div class="brand"><i>Olo</i> TV</div><div class="grow"></div><button class="iconbtn" onclick="refreshCurrent()" aria-label="Oppdater">↻</button><button class="iconbtn" onclick="openMore()" aria-label="Mer">•••</button></header>
<main id="home" class="screen on"><div class="hero"><div class="eyebrow">Din TVMate</div><h1 id="hello">God dag</h1><div class="muted">Det viktigste, tilpasset telefonen.</div></div><section class="section"><div class="sectionhead"><h2>Neste kamper</h2><div class="grow"></div><button class="pill" onclick="go('sport')">Se alle</button></div><div id="homeSport"><div class="loader"></div></div></section><section class="section"><div class="sectionhead"><h2>Favorittkanaler</h2></div><div id="homeChannels"></div></section><section class="section"><div class="sectionhead"><h2>Nye episoder</h2></div><div id="homeEpisodes"></div></section></main>
<main id="tv" class="screen"><div class="hero"><div class="eyebrow">Live TV</div><h1>Kanaler</h1></div><div class="search"><input id="tvQ" placeholder="Søk etter en kanal"><button onclick="loadTV()">Søk</button></div><div id="tvChannels"><div class="loader"></div></div></main>
<main id="sport" class="screen"><div class="hero"><div class="eyebrow">Sport</div><h1>Kamper og kanaler</h1></div><div class="search"><input id="sportQ" placeholder="Lag, f.eks. Leeds"><button onclick="searchSport()">Søk</button></div><div id="sportRows"><div class="loader"></div></div></main>
<main id="library" class="screen"><div class="hero"><div class="eyebrow">Bibliotek</div><h1>Filmer og serier</h1></div><div class="chips"><button class="chip on" data-lib="movies" onclick="libraryTab(this)">Filmer</button><button class="chip" data-lib="shows" onclick="libraryTab(this)">Serier</button></div><div class="search"><input id="libraryQ" placeholder="Søk"><button onclick="searchLibrary()">Søk</button></div><div id="libraryRows"><div class="loader"></div></div></main>
<main id="playlist" class="screen"><div class="hero"><div class="eyebrow">Playlist Builder</div><h1>Finn kanaler</h1></div><div class="search"><input id="playlistQ" placeholder="f.eks. V Sport"><button onclick="searchPlaylist()">Søk</button></div><div id="playlistRows"></div><section class="section"><div class="sectionhead"><h2>Kategorier</h2></div><div id="categoryRows"><div class="loader"></div></div></section></main>
<main id="racing" class="screen"><div class="hero"><div class="eyebrow">Racing</div><h1>Løp og førere</h1></div><div id="racingRows"><div class="loader"></div></div></main>
<main id="games" class="screen"><div class="hero"><div class="eyebrow">Spill</div><h1>Ønskeliste</h1></div><div id="gameRows"><div class="loader"></div></div></main>
<main id="settings" class="screen"><div class="hero"><div class="eyebrow">Innstillinger</div><h1>Olo TVMate</h1><div class="muted">Mobilvisningen lagrer de samme innstillingene som PC-appen.</div></div><div id="settingsRows"><div class="loader"></div></div></main>
</div>
<nav class="bottom"><button class="nav on" data-go="home"><b>⌂</b>Hjem</button><button class="nav" data-go="tv"><b>▣</b>Live TV</button><button class="nav" data-go="sport"><b>⚽</b>Sport</button><button class="nav" data-go="library"><b>▶</b>Bibliotek</button><button class="nav" onclick="openMore()"><b>☰</b>Mer</button></nav>
<div id="sheetback" class="sheetback" onclick="closeMore(event)"><div class="sheet"><div class="handle"></div><button class="sheetitem" onclick="go('playlist')"><b>☷</b>Playlist Builder</button><button class="sheetitem" onclick="go('racing')"><b>🏁</b>Racing</button><button class="sheetitem" onclick="go('games')"><b>🎮</b>Spill</button><button class="sheetitem" onclick="go('settings')"><b>⚙</b>Innstillinger</button><button class="sheetitem" onclick="location.href='/desktop'"><b>↗</b>Åpne PC-visning</button></div></div>
<div id="mplayer" class="mplayer hide" onclick="if(event.target===this)closePlayer()"><div class="mplayerbox"><div class="mplayerbar"><span id="mplayerTitle">Spiller</span><button class="mplayerclose" onclick="closePlayer()">&times;</button></div><video id="mvideo" controls autoplay playsinline webkit-playsinline></video><div id="mplayerMsg" class="mplayermsg"></div><div class="mplayeractions"><button class="action vlc" id="mplayerVlc">Åpne i VLC</button></div></div></div>
<script>
const S={screen:'home',library:'movies',loaded:{}};const $=id=>document.getElementById(id);const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
async function api(path,opt){const r=await fetch(path,opt);let j={};try{j=await r.json()}catch(e){}if(!r.ok)throw Error(j.error||r.status);return j}
function empty(t){return '<div class="empty">'+esc(t)+'</div>'}function art(src,cls=''){return src?'<img class="art '+cls+'" src="'+esc(src)+'" loading="lazy" onerror="this.remove()">':'<div class="art '+cls+'"></div>'}
function openMore(){$('sheetback').classList.add('on')}function closeMore(e){if(!e||e.target===$('sheetback'))$('sheetback').classList.remove('on')}
function go(id){closeMore();document.querySelectorAll('.screen').forEach(x=>x.classList.toggle('on',x.id===id));document.querySelectorAll('.nav[data-go]').forEach(x=>x.classList.toggle('on',x.dataset.go===id));S.screen=id;scrollTo(0,0);loadScreen(id)}
document.querySelectorAll('.nav[data-go]').forEach(b=>b.onclick=()=>go(b.dataset.go));
function fmtDate(v){if(!v)return '';const d=new Date(v);return isNaN(d)?v:d.toLocaleString('nb-NO',{weekday:'short',day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'})}
function streamActions(c){const sid=c.stream_id;if(sid==null)return '';return '<div class="actions"><button class="action primary" onclick="play('+JSON.stringify(String(sid))+')">▶ Spill</button><button class="action vlc" onclick="vlc('+JSON.stringify(String(sid))+')">VLC</button></div>'}
let _mhls=null,_mstream=null;
function closePlayer(){const m=$('mplayer'),v=$('mvideo');if(_mhls){try{_mhls.destroy()}catch(e){}_mhls=null}try{v.pause();v.removeAttribute('src');v.load()}catch(e){}m.classList.add('hide');_mstream=null}
async function play(id){
  const m=$('mplayer'),v=$('mvideo'),msg=$('mplayerMsg');
  $('mplayerTitle').textContent='Spiller';msg.textContent='Laster…';m.classList.remove('hide');
  if(_mhls){try{_mhls.destroy()}catch(e){}_mhls=null}
  let urls;
  try{urls=await api('/api/hls?id='+encodeURIComponent(id));if(urls.error||!urls.hls)throw 0;}
  catch(e){msg.textContent='Kunne ikke bygge strøm-URL.';return;}
  _mstream=String(id);
  $('mplayerVlc').onclick=function(){vlc(id);};
  startMobileStream(v,urls,s=>msg.textContent=s);
}
function startMobileStream(video,urls,status){
  let stopped=false;
  function viaProxy(u){return '/api/proxy?u='+encodeURIComponent(u);}
  function tryHls(src,proxied){
    if(window.Hls&&Hls.isSupported()){
      status(proxied?'Kobler til…':'Laster HLS…');
      _mhls=new Hls({manifestLoadingTimeOut:12000,fragLoadingTimeOut:20000,backBufferLength:30,maxBufferLength:45});
      _mhls.on(Hls.Events.ERROR,function(ev,data){
        if(stopped||!data.fatal)return;
        if(!proxied){try{_mhls.destroy()}catch(e){}_mhls=null;tryHls(viaProxy(urls.hls),true);return;}
        status('Kunne ikke spille i appen. Prøv VLC.');
      });
      _mhls.on(Hls.Events.MANIFEST_PARSED,function(){status('');const p=video.play();if(p&&p.catch)p.catch(()=>{});});
      _mhls.loadSource(src);_mhls.attachMedia(video);return;
    }
    if(video.canPlayType('application/vnd.apple.mpegurl')){
      status('Laster HLS…');video.src=src;
      video.addEventListener('error',function h(){video.removeEventListener('error',h);if(!proxied)tryHls(viaProxy(urls.hls),true);else status('Kunne ikke spille i appen. Prøv VLC.');},{once:true});
      const p=video.play();if(p&&p.catch)p.catch(()=>{});return;
    }
    status('Kunne ikke spille i appen. Prøv VLC.');
  }
...
async function loadSport(){try{const r=await api('/api/my_teams');let fx=(r.fixtures||[]).filter(x=>!x.is_finished);
  // Cap at 3 upcoming fixtures per favourite team so one team can't flood the
  // list. Fixtures are already sorted by start time, so the earliest 3 win.
  const perTeam={},capped=[];
  fx.forEach(function(f){
    const teams=(f.favorite_teams&&f.favorite_teams.length)?f.favorite_teams:[(f.home||'')];
    // A fixture counts toward each of its teams; keep it if ANY team is under 3.
    let keep=false;
    teams.forEach(function(t){const k=String(t).toLowerCase();perTeam[k]=(perTeam[k]||0)+1;if(perTeam[k]<=3)keep=true;});
    if(keep)capped.push(f);
  });
  $('sportRows').innerHTML=capped.map(f=>fixtureCard(f)).join('')||empty('Ingen kommende kamper.')}catch(e){$('sportRows').innerHTML=empty('Kunne ikke laste kamper.')}}
async function searchSport(){const q=$('sportQ').value.trim();if(!q)return loadSport();$('sportRows').innerHTML='<div class="loader"></div>';try{const r=await api('/api/search?q='+encodeURIComponent(q));$('sportRows').innerHTML=(r.fixtures||[]).map(f=>fixtureCard(f)).join('')||empty('Ingen kamper funnet.')}catch(e){$('sportRows').innerHTML=empty('Søket feilet.')}}
async function checkFixture(btn){const card=btn.closest('.card'),all=[...$('sportRows').querySelectorAll('.card')],idx=all.indexOf(card);let data;try{const r=await api('/api/my_teams');data=(r.fixtures||[]).filter(x=>!x.is_finished)[idx];if(!data)return;btn.textContent='Sjekker…';const x=await api('/api/sports_event_channels',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fixture:data})});card.outerHTML=fixtureCard(Object.assign(data,x));}catch(e){btn.textContent='Prøv igjen'}}
async function loadTV(){const q=$('tvQ').value.trim();$('tvChannels').innerHTML='<div class="loader"></div>';try{let rows;if(q){rows=(await api('/api/channels?q='+encodeURIComponent(q)+'&cat=')).channels||[]}else{rows=(await api('/api/favorites')).channels||[]}$('tvChannels').innerHTML=rows.slice(0,100).map(c=>'<div class="card">'+channelRow(c)+'</div>').join('')||empty(q?'Ingen kanal funnet.':'Ingen favorittkanaler.')}catch(e){$('tvChannels').innerHTML=empty('Kunne ikke laste kanaler.')}}
function libraryTab(b){document.querySelectorAll('[data-lib]').forEach(x=>x.classList.toggle('on',x===b));S.library=b.dataset.lib;loadLibrary()}
function mediaTile(x){
  const sid=(x.stream_id!=null&&(x.stream_found!==false))?String(x.stream_id):'';
  const onclick=sid?(' onclick="play('+JSON.stringify(sid)+')"'):'';
  const cls=sid?'tile tileplay':'tile';
  const badge=sid?'<div class="tilebadge">&#9654;</div>':'';
  return '<div class="'+cls+'"'+onclick+'>'+(x.cover?'<img src="'+esc(x.cover)+'" loading="lazy">':'<div style="aspect-ratio:2/3"></div>')+badge+'<div class="title">'+esc(x.name||x.show_name||'')+'</div><div class="meta">'+esc(x.year||x.episode_name||'')+'</div></div>'}
async function loadLibrary(){try{let rows;if(S.library==='movies')rows=(await api('/api/movie_catalog?catalog=popular&limit=18')).movies||[];else rows=(await api('/api/favorites')).shows||[];$('libraryRows').innerHTML='<div class="grid">'+rows.map(mediaTile).join('')+'</div>'||empty('Ingen innhold.')}catch(e){$('libraryRows').innerHTML=empty('Kunne ikke laste biblioteket.')}}
async function searchLibrary(){const q=$('libraryQ').value.trim();if(!q)return loadLibrary();$('libraryRows').innerHTML='<div class="loader"></div>';try{const r=await api((S.library==='movies'?'/api/movies?q=':'/api/shows?q=')+encodeURIComponent(q));const rows=S.library==='movies'?(r.movies||[]):(r.shows||[]);$('libraryRows').innerHTML=rows.length?'<div class="grid">'+rows.map(mediaTile).join('')+'</div>':empty('Ingen treff.')}catch(e){$('libraryRows').innerHTML=empty('Søket feilet.')}}
async function loadPlaylist(){try{const r=await api('/api/categories');$('categoryRows').innerHTML='<div class="categories">'+(r.categories||[]).slice(0,150).map(c=>'<button class="category" onclick="openCategory('+JSON.stringify(c.name)+')"><span>'+esc(c.name)+'</span><span>'+esc(c.count)+'</span></button>').join('')+'</div>'}catch(e){$('categoryRows').innerHTML=empty('Kunne ikke laste kategorier.')}}
async function searchPlaylist(){const q=$('playlistQ').value.trim();if(!q)return;$('playlistRows').innerHTML='<div class="loader"></div>';try{const r=await api('/api/channels?q='+encodeURIComponent(q)+'&cat=');$('playlistRows').innerHTML=(r.channels||[]).slice(0,100).map(c=>'<div class="card">'+channelRow(c)+'</div>').join('')||empty('Ingen treff.')}catch(e){$('playlistRows').innerHTML=empty('Søket feilet.')}}async function openCategory(name){$('playlistRows').innerHTML='<div class="loader"></div>';const r=await api('/api/channels?q=&cat='+encodeURIComponent(name));$('playlistRows').innerHTML='<div class="sectionhead"><h2>'+esc(name)+'</h2></div>'+(r.channels||[]).slice(0,100).map(c=>'<div class="card">'+channelRow(c)+'</div>').join('');scrollTo(0,0)}
async function loadRacing(){try{const [r,d]=await Promise.all([api('/api/racing'),api('/api/racing_drivers')]);const events=r.events||[],drivers=d.drivers||[];$('racingRows').innerHTML=events.map(x=>'<div class="card"><div class="row"><div class="grow"><div class="title">'+esc(x.name||x.event||x.series_name||'Løp')+'</div><div class="meta">'+esc(x.series_name||x.series||'')+' · '+esc(fmtDate(x.start))+'</div></div>'+(x.is_live?'<span class="tag live">LIVE</span>':'')+'</div></div>').join('')+(drivers.length?'<div class="sectionhead"><h2>Førere</h2></div>'+drivers.map(x=>'<div class="card"><div class="row">'+art('/api/racing_driver_image?id='+encodeURIComponent(x.key||''))+'<div><div class="title">'+esc(x.name)+'</div><div class="meta">'+esc(x.team||x.series_name||'')+'</div></div></div></div>').join(''):'')||empty('Ingen racingdata.')}catch(e){$('racingRows').innerHTML=empty('Kunne ikke laste racing.')}}
async function loadGames(){try{const r=await api('/api/favorites'),rows=r.games||[];$('gameRows').innerHTML=rows.map(x=>'<div class="card"><div class="row">'+art(x.image||x.logo)+'<div><div class="title">'+esc(x.name||x.title||'Spill')+'</div><div class="meta">'+esc(x.release_date||x.status||'')+'</div></div></div></div>').join('')||empty('Ingen spill i ønskelisten.')}catch(e){$('gameRows').innerHTML=empty('Kunne ikke laste spill.')}}
async function loadSettings(){try{const c=await api('/api/config');$('settingsRows').innerHTML='<div class="card" style="padding:15px"><div class="field"><label>Profilnavn</label><input id="cfgName" value="'+esc(c.profile_name||'Olo')+'"></div><div class="field"><label>Språk</label><select id="cfgLang"><option value="no">Norsk</option><option value="en">English</option></select></div><div class="field"><label><input id="cfgLan" type="checkbox" '+(c.allow_lan?'checked':'')+'> Tillat lokal Wi-Fi</label></div><button class="save" onclick="saveSettings()">Lagre</button><div class="meta" style="margin-top:13px">Versjon __VERSION__</div></div>';if($('cfgLang'))$('cfgLang').value=c.preferred_language||'no'}catch(e){$('settingsRows').innerHTML=empty('Kunne ikke laste innstillinger.')}}
async function saveSettings(){try{const old=await api('/api/config');old.profile_name=$('cfgName').value.trim()||'Olo';old.preferred_language=$('cfgLang').value;old.allow_lan=$('cfgLan').checked;const r=await api('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(old)});alert(r.ok===false?'Kunne ikke lagre.':'Lagret.')}catch(e){alert('Kunne ikke lagre.')}}
function loadScreen(id,force){if(S.loaded[id]&&!force)return;S.loaded[id]=true;if(id==='home')loadHome(force);if(id==='tv')loadTV();if(id==='sport')loadSport();if(id==='library')loadLibrary();if(id==='playlist')loadPlaylist();if(id==='racing')loadRacing();if(id==='games')loadGames();if(id==='settings')loadSettings()}
function refreshCurrent(){S.loaded[S.screen]=false;loadScreen(S.screen,true)}
$('tvQ').addEventListener('keydown',e=>{if(e.key==='Enter')loadTV()});$('sportQ').addEventListener('keydown',e=>{if(e.key==='Enter')searchSport()});$('playlistQ').addEventListener('keydown',e=>{if(e.key==='Enter')searchPlaylist()});$('libraryQ').addEventListener('keydown',e=>{if(e.key==='Enter')searchLibrary()});loadHome();S.loaded.home=true;
</script></body></html>'''
...
def test_external_source(key):
    ...
    probes = {
        ...
        "vg_tvguide": lambda: fetch_vg_channel_catalog(),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if u.path == "/api/sports_competitions":
                cfg = load_config()
                return self._send(200, {"groups": SPORTS_COMPETITIONS,
            ...
            if u.path == "/api/my_teams":
                ...
                for favorite in favorites:
                    ...
                    # The Sports home only needs the next match (or a busy
                    # week's two/three). The complete schedule remains on the
                    # dedicated team page; channel matching there starts only
                    # when that team page is actually opened.
                    errors.extend(add_primary_tv_listings(
                        compact_team_fixture_window(fixtures), countries))
                    for fixture in fixtures:
                ...
                try:
                    top_fixtures = featured_daily_fixtures(cfg.get("sports_competitions"))
            ...
            if u.path == "/api/search":
                ...
                if logged_in:
                    ...
                    vg_discoveries = _vg_fixture_discoveries(
                        fixtures, channels, cats, x)
                out = []
                for f in fixtures:
                    matches = []
                    ...
                    if logged_in:
                        ...
                        if not cached:
                            ...
                            if (isinstance(stored, dict) and
                                ...
                                cached = {
                                    ...
                                    "result": _enforce_fixture_secure_matches(
                                        f, _sports_result_for_client(
                                            stored["result"], x)),
                                }
                        ...
                        result = _add_epg_discoveries(
                            result, vg_discoveries.get(disk_key, []))
                        result = _enforce_fixture_secure_matches(f, result)
    ...
    def do_POST(self):
        ...
        if u.path == "/api/sports_event_channels":
            ...
            league_name = str(fixture.get("league_name") or "").strip()[:160]
            ...
            if fresh and not payload.get("force"):
                result = _enforce_fixture_secure_matches(
                    {"home": home, "away": away, "start": start,
                     "league_name": league_name, "by_country": cleaned_tv},
                    cached.get("result") or {})
                return self._send(200, dict(result, cached=True))
            if payload.get("cached_only"):
                return self._send(200, {"cached": False})
            try:
                result = find_sports_event_channels(
                    {"home": home, "away": away, "start": start,
                     "league_name": league_name, "by_country": cleaned_tv}, cfg)
        ...
        if u.path == "/api/sports_availability":
            ...
            for raw_fixture in incoming[:160]:
                ...
                league_name = str(raw_fixture.get("league_name") or "").strip()[:160]
                ...
                result = _enforce_fixture_secure_matches(
                    {"home": home, "away": away, "start": start,
                     "league_name": league_name, "by_country": cleaned_tv}, result)
                _SPORTS_EVENT_CHANNEL_CACHE[key] = {"ts": time.time(), "result": result}
        ...
        if u.path == "/api/config":
            ...
            for k in ("xtream_host", "xtream_port", "xtream_user", "xtream_pass",
                      ...
                      "preferred_language", "profile_emblem", "mylist_layout", "sports_layout", "sports_competitions", "match_layout", "football_enabled",
...
def run_self_tests():
    ...
    check("Sports layout defaults safely and exposes all profile choices",
          DEFAULT_CONFIG.get("sports_layout") == "current" and
          all(('value="%s"' % layout) in PAGE
              for layout in ("current", "broadcast", "agenda", "hub", "timeline")) and
          all(("sports-layout-" + layout) in PAGE
              for layout in ("broadcast", "agenda", "hub", "timeline")))
    compact_sample = [
        {"start": datetime.datetime.fromtimestamp(1000 + offset,
            datetime.timezone.utc).isoformat(), "home": "A", "away": str(offset)}
        for offset in (3600, 2 * 86400, 5 * 86400, 9 * 86400)]
    ...
    check("live centre anchors the player stack while channel hits scroll",
          ...
          ".matchdetailsticky{grid-column:1;grid-row:1;position:sticky" in PAGE and
          ".matchdetailchannels{grid-column:2;grid-row:1" in PAGE and
    ...
    check("fixture selection opens a dedicated match page",
    ...
    check("sports refresh APIs preserve league before secure enforcement",
          source_text.count(
              'league_name = str(fixture.get("league_name") or "").strip()[:160]') >= 1 and
          source_text.count(
              'league_name = str(raw_fixture.get("league_name") or "").strip()[:160]') >= 1 and
          source_text.count('"league_name": league_name') >= 4)
    cache_reclassified = _enforce_fixture_secure_matches(
        competition_fixture, _sports_result_for_client({
            "matches": [], "ppv_hits": [{
                "stream_id": 4023,
                "xtream_name": "NO: V SPORT PREMIER LEAGUE 3 RAW NO",
                "category": "NO| NORWAY HD/RAW"}]}, _CompetitionTestX()))
    check("cached Premier League rows are reclassified when read",
          cache_reclassified["ppv_hits"][0].get("secure_reason") ==
          "norway_premier_league")
    ...
    vg_payload = _vg_next_data(
        '<script id="__NEXT_DATA__" type="application/json">'
        '{"props":{"pageProps":{"channels":[{"name":"V sport Premier League 3",'
        '"slug":"v-sport-premier-league-3"}]}}}</script>')
    check("VG Next.js guide payload parses without browser automation",
          vg_payload.get("channels", [{}])[0].get("slug") ==
          "v-sport-premier-league-3")
    check("VG guide channel aliases map to Norwegian playlist spelling",
          _vg_channel_key("NO: V Sport PL 3 UHD") ==
          _vg_channel_key("V sport Premier League 3"))
    ...
    plain_viaplay = match_channels(
        {"NO": ["Viaplay"], "FI": ["Viaplay"]},
        [{"name": "NO: V Sport Live 3", "stream_id": 126, "category_id": "no"},
         {"name": "FI: V Sport+ Suomi", "stream_id": 127, "category_id": "fi"},
         {"name": "4K: V Sport+", "stream_id": 128, "category_id": "4k"}],
        {"no": "NO | SPORTS", "fi": "FI | SPORTS",
         "4k": "4K | UHD CHANNELS"}, 0.62)
    check("country-scoped plain Viaplay expands to Nordic V Sport feeds",
          {row["stream_id"] for row in plain_viaplay} == {126, 127, 128})
    check("nearby LTV dates include third fixture after friendly and cup",
          _nearby_ltv_dates([
    ...
    racing_rows = find_racing_channels(
        ...
        [{"name": "F1 Dutch Grand Prix 4K", "stream_id": 20, "category_id": "4k"},
         ...
         {"name": "NO: V SPORT 1 VIP RAW", "stream_id": 23, "category_id": "no"},
         {"name": "NO: V SPORT 2", "stream_id": 24, "category_id": "no"},
         {"name": "NO: VIAPLAY PPV 4", "stream_id": 25, "category_id": "no"},
    ...
    check("Norwegian V Sport 1 is a confirmed F1 broadcaster",
          racing_kinds.get(23) == "broadcaster" and 24 not in racing_kinds)
    check("F1 Viaplay fallback requires PPV in the channel name",
          racing_kinds.get(25) == "possible" and
    ...
    wrc_callsign_rows = find_racing_channels(
        {"series": "wrc", "race": "Rally del Paraguay", "circuit": "",
         "session": "Rally weekend"},
        [{"name": "US: NBC (WRC) WASHINGTON DC (D)", "stream_id": 44,
          "category_id": "us"},
         {"name": "US: NBC 4 (WRC) LEESBURG (H)", "stream_id": 45,
          "category_id": "us"},
         {"name": "US: NBC 4 (WRC) WASHINGTON (A)", "stream_id": 46,
          "category_id": "us"},
         {"name": "ES: RALLY TV UHD", "stream_id": 47,
          "category_id": "racing"}],
        dict(sample_cats, us="US Local", racing="Racing"), _TestRacingXtream())
    wrc_callsign_ids = {row["stream_id"] for row in wrc_callsign_rows}
    check("WRC callsign collision excludes Washington NBC affiliates",
          wrc_callsign_ids == {47})
